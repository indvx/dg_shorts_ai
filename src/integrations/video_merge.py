...
class VideoMergeService(BaseService):
    ...
    def merge_and_mute_video(self, video):

        os.makedirs(self.output_directory, exist_ok=True)
        os.makedirs(self.music_directory, exist_ok=True)
        output_path = video.background_video_url.replace("video", "output")
        video_clip = None
        audio_clip = None
        final_clip = None
        music_clip = None
        ...
        try:
            ...
            base_video = VideoFileClip(video.background_video_url).without_audio()
